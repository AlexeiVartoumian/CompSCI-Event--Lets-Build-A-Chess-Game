...
argparser = argparse.ArgumentParser(description = "silly content tracker")
...
class GitRepository(object):
    ...
    def repo_create(path):
        ...
        if os.path.exists(repo.getdir) and os.listdir(repo.gitdir):
...
def log_graphviz(repo , sha , seen):
    ...
    if "\n" in message: # keep only first line
...
def tree_parse_one(raw , start = 0):
    # find the space terminator fo mode
    x = raw.find(b' ', start)
    assert x-start == 4 or x-start == 6
    ...
    # read sha and then convert it to a hex string
    sha = format(int.from_bytes(raw[y+1:y+21] , "big"), "040x")
    return  y+21 , GitTreeLeaf(mode, path.decode("utf8"), sha)

#"real parser calls above in loop"
def tree_parse(raw):
    pos = 0
    max=  len(raw)
    ret = list()
...
def tree_leaf_sort_key(leaf):
    if leaf.mode.startswith(b'10'): # the mode
        return leafpath
    else:
        return leaf.path + "/"
...
argsp.add_arguemt("tree", help = "a tree-ish object")
    ...
    for item in obj.items:
        ...
        if not (recursive and type == 'tree'): 
            print("{0} {1} {2}\t{3}".format(
                ...
                #gits s tree displays the type
                # of object pointed to. we can do that too
                type,
                item.sha,
...
def cmd_checkout(args):
    ...
    #if object is commit then rgab the tree
    if obj.fmt == b'commit':
...
def tree_checkout(repo, tree, path):
    ...
    for item in tree.items:
        ...
        if obj.fmt == b'tree':
            ...
            # does not support file mode 12 for symlink
            with open(dest, 'wb') as f:
                f.write(obj.blobdata)

#7.1 refs to heaxdecminal strings on braches
...
argsp.add("-a" , action = "store_true",
dest = "create_tag_object",
help = "Whether to cerate a tag object")
...
def tag_create(repo , name , ref , create_tag_object= False):
    # get gitobject from object reference
    ...
    if create_tag_object:
        ...
        tag.kvlm[None] = b"A tag genreated by wyag which wont customise the message"
...
This function is aware of:
 ...
 - the HEAD literal
    ...
    #empty str then abort
    if not name.strip():
        return NOne
    
    # head is non-ambigous
    if name == "HEAD":
        return [ref_resolve(repo, "HEAD")]
    
    # if its a hex rstring then try for hash
    if hashRE.match(name):
        ...
        as_branch = ref_resolve(repo , "res/heads/" + name)
        if as_bramch: # brnach found ?
...
class GitIndexEntry(object):
    ...
    #the id of device conatint this dile
    self.dev = dev

    #the files inode number
    self.ino = ino
    #the obect type , eitehr b1000 , b1010(symlink), b1110 gitlink
    self.mode_type = mode_type
    
    # the objects permssions as an integer
    ...
    #group id of owenr
    self.gid = gid
    #size of object in buts
    self.fsize = fsize
    # the objects sha
    self.sha = sha
    self.flaf_assume_valid = flag_assume_valid
    self.flag_stage = falg_stage
    #name of object with full path
    self.name = name
...
def index_read(repo):
    ...
    for  i in range(0, count):
        #read the creation time as a unix timestamp seconds since 
        #1970 -01 -01 the singularituuuuu
        ctime_s = int.from_bytes(content[idx:idx+4],"big")
        #read creation time as nanseconds after time stamps for precision
        ctime_ns = int.from_bytes(content[idx+4: idx+8], "big")
        #now do modifcation time , first seconds from epocj
        ...
        mode_type = mode >>12
        asswer mode_tyype in [0b1000,0b1010,0b1110]
        mode_perms = mode & 0b0000000111111111
        #user id
        uid = int.from_bytes(content[idx+28:idx+32], "big")
        
        #group id 
        uid = int.from_bytes(content[idx+32:idx+36],"big")
        
        #size 
        fsize = int.from_bytes(content[idx+36:idx+40],"big")
        
        #sha object id sotre it as lowecase hex string fir consistency
        ...
        #parse flags
        flags_assume_valid = (flags & 0b1000000000000000) != 0
        ...
        idx = 8 * ceil(idx/8)
        #add the entory to our list

        entries.append(GitIndexEntry(ctime = (ctime_s,ctime_ns), mtime= (mtime_s,mtime_ns), dev = dev,
        ino = ino,
        mode_type = mode_type,
...
def cmd_ls_files(args):
    ...
    for e in index.entries:
        ...
        if args.verbose:
            ...
            print(" user: {} ({}) group: {} ({})".format(pwd.getpwuid(e.uid).pw_name, e.uid,grp.getgrgid(e.gid).gr_name,
            e.gid))
...
#8.4
argsp = argsubparsers.add_parser("check-ignore" , help = "Check paths(s) against ignore rules")
...
def gitignore_parse1(raw):
    raw = raw.strip() # remove leafing/ trailing spaces
...
def gitignore_read(repo):
    ...
    #read lcoal config in .git/info.exclude
